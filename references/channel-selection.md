...
> 本表是检索通道的**逐通道实测台账**（带实测日期），只回答"这个通道实测怎么走"，随实测更新；"已知被墙/未被墙境外/国内三类怎么决策、代理不可用怎么办"的分流原则按 security-baseline 安全基线，本表不重复。

| 目标 | 通道 |
| --- | --- |
| Google、YouTube、X 等被墙站点 | 直接开代理、不做直连探测（端口以本机实测为准：ClashX 多为 7890、ClashVerge 多为 7897） |
